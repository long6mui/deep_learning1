...
% Title
{\fontsize{32pt}{1}\selectfont \textbf{REPORT}}\\[0.4cm]
{\fontsize{20pt}{1}\selectfont Introduction to Machine Learning}\\[0.8cm]
\fontsize{15pt}{1}\selectfont \textbf{LibRAG: }}\\[1.8cm]
{\Large \textbf{}}\\[6pt]
{\large \textbf{Supervisors:}  PGS. Phạm Văn Hải}\\
{\textbf{Class: }}
\vfill
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|c|l|c|X|} 
        ...
        \hline
        1 & Nguyễn Văn Hưởng  & 2023xxxx & huong.nv23xxxx@sis.hust.edu.vn \\
        \hline
        2 & Trần Minh Khoa    & 2024002  & khoa.tm24002@sis.happroachust.edu.vn \\
        \hline
        3 & Nguyễn Minh Khôi  & 2024003  & Khoi.NM235123@sis.hust.edu.vn \\
        \hline
        4 & Trần Tiến Dũng    & 2024004  & dung.tt24004@sis.hust.edu.vn \\
        \hline
        5 & Nguyễn Đình Hoàng & 2023xxx  & hoang.nd23xxx@sis.hust.edu.vn \\
        \hline
    \end{tabularx}
\end{table}
% Supervisors
...
\section{Motivation}
The unprecedented evolution of Large Language Models (LLMs) has fundamentally redefined the boundaries of Natural Language Processing. These models have become ubiquitous, endowing artificial agents with the capacity to comprehend and generate human-like text with remarkable fluency \cite{devlin-etal-2019-bert, touvron2023llama}. In the context of \textbf{Library and Information Science}, specifically bibliographic discovery and book recommendation, this advancement offers a significant opportunity to transform how patrons interact with \textbf{digital library catalogs}. Users no longer seek simple keyword-based results; they demand intelligent agents capable of understanding complex, context-rich queries such as tracing \textit{literary influences} or exploring thematic similarities across \textit{extensive book series}.

However, the bibliographic domain is characterized by high \textit{relational density}. Entities such as \textbf{authors, editors, genres, and publishing houses} are not isolated data points but are inextricably linked through a complex web of intellectual and historical connections. To fully leverage the potential of LLMs in this domain, there is a critical need for a framework that can not only retrieve information but also comprehend the \textbf{topological structure} of these relationships. This necessitates a paradigm shift towards Retrieval-Augmented Generation with Graphs (GraphRAG) \cite{han2025retrievalaugmentedgenerationgraphsgraphrag}, which utilizes Knowledge Graphs to preserve the semantic context necessary for answering sophisticated bibliographic queries.
...
\begin{figure}
    ...
    \caption{Example Graph in Neo4j}
    \label{fig:graph}
\end{figure}
...
\subsection{Overall Performance (Unweighted Analysis)}
...
\subsubsection{Analysis of Unweighted Scores}
Even under an unweighted evaluation schema, GraphRAG maintains its superiority over the baseline:

\begin{itemize}
    \item \textbf{Consistency:} The unweighted score for GraphRAG ($0.8116$) is statistically identical to the weighted score ($0.812$), indicating that the system's performance is balanced. It does not rely solely on heavily weighted metrics (like Faithfulness) to achieve a high overall score.
    \item \textbf{Trade-off Impact:} The drops in \textit{Relevancy} and \textit{Precision} (totaling $-0.034$) are effectively offset by the substantial gains in \textit{Faithfulness} ($+0.122$) and \textit{Recall} ($+0.033$).
    \item \textbf{Conclusion:} Whether prioritizing factual safety (Weighted) or general retrieval quality (Unweighted), GraphRAG consistently delivers a $\sim3.5\%$ performance improvement over standard vector-based RAG.
\end{itemize}
...
\begin{tabular}{lcccl}
\toprule
\textbf{Dataset} & \textbf{GraphRAG} & \textbf{SimpleRAG} & \textbf{$\Delta$} & \textbf{Winner} \\ \midrule
